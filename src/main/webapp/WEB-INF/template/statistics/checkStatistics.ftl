...
<body>
	<div class="path">
		<a href="${base}/admin/index/">${message("admin.path.index")}</a> &raquo; 审核人工作统计 <span>(${message("admin.page.total", pager.totalCount)})</span>
	</div>
	<form id="listForm" action="checkStatistics" method="get">
		...
		<table id="listTable" class="list">
			<tr>
				...
				<th>
					<a href="javascript:;" class="sort" name="name">操作员</a>
				</th>
				<th>
			...
			[#list data as checkStatistics]
				<tr>
					<td width="2%">
						${checkStatistics_index+1}
					</td>
					<td>
						${checkStatistics.name}
					</td>
					<td>
						${checkStatistics.checkNum}
					</td>
					<td>
						${checkStatistics.successNum}
					</td>
				</tr>
			[/#list]
		</table>
