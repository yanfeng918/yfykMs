...
<body>
	...
	<form id="inputForm" action="update" method="post">
		...
		<table class="input tabContent">
			...
			<tr>
				...
				<td>
					...
					<input type="hidden" id="areaId" name="areaId" value="${(houseInfo.area.id)!}" treePath="${(houseInfo.area.treePath)!}" />
				</td>
			</tr>
		...
		<tr>
			...
			<td>
				<select name="status">
					<option value="CHECKING" [#if (houseInfo.status)?? &&(houseInfo.status)=="CHECKING"] selected="selected"[#else][/#if]>审核中</option>
					<option value="FAIL" [#if (houseInfo.status)?? &&(houseInfo.status)=="FAIL"] selected="selected"[#else][/#if]>失败</option>
					<option value="SUCCESS" [#if (houseInfo.status)?? &&(houseInfo.status)=="SUCCESS"] selected="selected"[#else][/#if]>成功</option>
				</select>
			</td>
		</tr>
